...
const Reedem: React.FC = () => {
  ...
  useEffect(() => {
    const addOffersToMyCredits = async () => {
      ...
      if (projectIds) {
        ...
        const response = await fetch(`https://carbon-relay-23a0f49f1c2f.herokuapp.com/auth/add-to-my-offers?projectIds=${projectIds}`, {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://carbon-relay-23a0f49f1c2f.herokuapp.com/auth/create-bid/${projectId}`, {
        method: "POST",
        headers: {
